...
static int get_num_from_buffer(char *p_start, const char *p_end, long  *p_number);
...
static  int validate_id(char *p_start, char *p_end, long  *p_student_id);
...
int main(int argc, char **argv)
{
    ...
    do {
        ...
        student_list = (Student*) malloc(sizeof(Student)*n);
...
static void  best_student(Student *start, Student *end)
{
    ...
    while (start < end){
        if ((float)(start->grade)/(float)(start->age) >
        (float)(p_best->grade/(float)p_best->age))
        {
            p_best = start;
        }
    ...
    printf("best student info is: %ld,%d,%d\n",
           p_best->id,p_best->grade,p_best->age);
}
static int read_num_of_students(int *p_n)
{
    ...
    while (1)
    {
        ...
        while ( buffer[i] != '\n')
        {
            if ((buffer[i] < ZERO_DIGIT) || (buffer[i] > NINE_DIGIT))
            {
                flag = STATUS_ERROR;
...
static int get_num_from_buffer(char *p_start,
                               const char *p_end,
                               long  *p_number)
{
    ...
    while ( p_digit < p_end)
    {
        if ((*p_digit < ZERO_DIGIT) || (*p_digit > NINE_DIGIT))
        {
            return STATUS_ERROR;
        }
        p_digit++;
    ...
    *p_number = strtol(p_start, &p,BASE);
...
static  int validate_id(char *p_start, char *p_end, long  *p_student_id)
{
    ...
    if ((*p_start == ID_INVALID_PREFIX)
        || ((int)(p_end - p_start) != ID_LENGTH))
    {
        printf("ERROR: Id must have 10 digits and can not start with 0.\n");
        return STATUS_ERROR;
    }
