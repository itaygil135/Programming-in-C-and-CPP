...
void test_hash_map_at(void)
{
    ...
    */
    assert((map = hashmap_alloc(hash_char)) != NULL);
    num_of_pairs = 0;
    // search for key at empty map
    assert(hashmap_at(map,pairs[IND0]->key) == NULL);

    // insert pair and find int
    num_of_pairs++;
    assert(hashmap_insert(map,pairs[IND0]) == PASS);
    assert(pairs[IND0]->value_cmp(hashmap_at(map,pairs[IND0]->key),
                                  pairs[IND0]->value) == TRUE);

    // search for key at that does not exist at the map with size=1
    assert(hashmap_at(map,pairs[IND1]->key) == NULL);

    // insert 24 more pairs , and search for them
    for (int k = IND1; k < IND25; ++k)
    {
        num_of_pairs++;
        assert(hashmap_insert(map, pairs[k]) == PASS);
        assert(pairs[k]->value_cmp(hashmap_at(map,pairs[k]->key),
                                      pairs[k]->value) == TRUE);
    }

    // search for key at that does not exist at the map with size=1
    assert(hashmap_at(map,pairs[IND25]->key) == NULL);
...
void test_hash_map_erase(void)
{
    ...
    /* test #1:   insert and erase pairs when each one locate at a different
     * bucket
    ...
    */
    ...
    assert(map->size == num_of_pairs);
    assert(hashmap_get_load_factor(map) ==
        (float)(0) / (float)(HASH_MAP_INITIAL_CAP_DIV_2));
