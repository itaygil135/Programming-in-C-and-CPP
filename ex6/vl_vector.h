...
public:
    ...
    public:
        ...
        Iterator()=default;
        Iterator(const T* ptr) : m_ptr((T*)ptr) {}
    ...
    const_iterator cbegin() const { return const_iterator(&m_vector[0]); }

    const_iterator cend() const { return const_iterator(&m_vector[m_size]); }
    ...
    reverse_iterator rend() { return reverse_iterator(m_vector - 1); }
                    ...
                    InputIterator last)
    {
       iterator n_pos;
       n_pos = &(*position);
       return insert(n_pos, first, last);
    }
    ...
    iterator insert(iterator position, InputIterator first, InputIterator last)
    {
        ...
        for (auto pointer = rbegin ();
            ...
            ++pointer)
        {
            dest[new_size - 1 - counter] = *pointer;
            counter++;
        }


        std::copy(first,last,&dest[size_to_position]);
